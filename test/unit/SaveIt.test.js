...
describe("SaveIt Unit Tests", function () {
  let saveit, vrfCoordinatorV2Mock, interval, mockV3Aggregator;
  ...
  describe("donating", () => {
    ...
    it("different address makes another donation", async () => {
      ...
      await expect(saveit.connect(donator5).donate({ value: fifthDonation }))
  ...
  // saveit tests
  describe("registering a single new food place", () => {
  ...
  describe("registering multiple new food places", () => {
    let numOfFoodPlaces;
    ...
    it("checks the locations of the food places", async () => {
      location1 = await saveit
        .connect(foodplace1)
        .getLocation(foodplace1.address);
      location2 = await saveit
        .connect(foodplace2)
        .getLocation(foodplace2.address);
      location3 = await saveit
        .connect(foodplace3)
        .getLocation(foodplace3.address);
      location4 = await saveit
        .connect(foodplace4)
        .getLocation(foodplace4.address);
  ...
  describe("request a new food delivery", () => {
  ...
  describe("Funding of a delivery", () => {
    let originalBalance;
    beforeEach(async () => {
      await deployments.fixture(["all"]);
      donation = await saveit.getUsdAmountInEth(10);
    ...
    it("handles the request", async () => {
      expect(await saveit.connect(foodplace1).requestDelivery(30)).to.emit(
        saveit,
        "RequestFunded"
      );
    });
  ...
  describe("Adding multiple foodie items", () => {
