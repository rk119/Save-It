...
describe("Donate contract", function () {
    ...
    before(async function () {
        ...
        [deployer, donator1, donator2] = accounts;
        donate = await Donate.new();
    });
    ...
    describe('donating', async () => {
        ...
        before(async () => {
            ...
            donation = web3.utils.toWei('1', 'Ether')
        ...
        it('check mapping', async () => {
            let donateBalance = await donate.addressToAmount(donator1)
        ...
        it('same address makes another donation', async () => {
            ...
            assert.equal(totalDonations, web3.utils.toWei('2.5', 'Ether'), 'total donations incremented correctly')
        ...
        it('different address makes another donation', async () => {
            ...
            let secondDonation = web3.utils.toWei('2', 'Ether')
        ...
        it('fails if donation amount is less than minimum', async () => {
            await donate.donate({ from: donator1, value: web3.utils.toWei('0.5', 'Ether') }).should.be.rejected
        })
    })
})
